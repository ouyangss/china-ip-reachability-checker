...
> `FAIL_THRESHOLD` 表示本轮运行内的失败阈值。脚本不会跨 cron / 跨运行保留失败计数；每次启动都会从 0 开始，结束前也会清零。默认 `1` 表示本轮判定国内不可达后立即调用换 IP 接口。
...
- `0`：国内可达
- `1`：国内不可达，本轮失败计数增加；达到 `FAIL_THRESHOLD` 后换 IP
- `2`：检测异常或结果矛盾，失败计数清零、不换 IP

为节省 Boce 余额，每轮脚本最多创建一次检测任务、获取一次结果，不做轮询。

脚本不保留跨运行失败计数。每次运行开始时失败计数为 `0`，运行结束前也会清零。
